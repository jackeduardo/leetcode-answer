...
func pathSum(root * Types.TreeNode, sum int) [][]int {
	var res [][]int
	var temp []int
	pathSum_helper(root,sum,&res,&temp)
	return res
}
// 类似回溯，一层函数的功能，再找递归位置
func pathSum_helper(root * Types.TreeNode, sum int, res*[][]int,temp *[]int){
	if root==nil{
		return
	}
	sum-=root.Val
	*temp=append(*temp,root.Val)
	...
	pathSum_helper(root.Left,sum,res,temp)
	pathSum_helper(root.Right,sum,res,temp)
