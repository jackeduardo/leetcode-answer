...
//所以任何涉及到append函数的slice都要考虑指针，包括全局变量，因为传递指针的话，虽然函数内的指针也是拷贝，但是指向的是同一个内存区域，那么对于*nums的append是会影响到原来数组的，而原来的数组也会被重新分配一个内存地址，
//这是append的特性
//所以append的本质就是给当前数组添加一个元素或多个元素，并重新给当前的变量分配一个新的地址,算是一种基于深拷贝的操作
