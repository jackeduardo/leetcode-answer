...
func solveSudoku(board [][]byte)  {
	...
	backtrack(board,row,col,block,0,0)
}
//由于每一步（填充）的前进都要判断是否符合条件，并且影响着前一步是否该放弃，所以此函数返回值应该是bool，void的方法我还没有想出来
func backtrack(board [][]byte,row [9][9]bool,col [9][9]bool,block [9][9]bool,i int,j int) bool{
	...
	if board[i][j]=='.' {
		for n := 0; n<9; n++ {
			...
			if !row[i][n]&&!col[j][n]&&!block[blockindex][n] {
				...
				if backtrack(board,row,col,block,i,j+1){
					return true
				}
				board[i][j]='.'
				row[i][n]=false
		...
		return backtrack(board,row,col,block,i,j+1)
	}
	return false
}